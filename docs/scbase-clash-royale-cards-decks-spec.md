...
状态：已完成。卡牌百科已通过全量目录、详情内容、数据审计和响应式验收；原卡组库方向已废弃，仅保留底层组件验证结果。
...
- [x] 将卡牌 Hugo 页面入口统一迁移到 `content/clashroyale/cards/`，数据继续保存在 `data/clashroyale/`。
- [x] 完成卡牌列表筛选、详情页双栏参数区、形态导航、版本历史和移动端布局的第一轮 UI 验收。
...
- [ ] 将 126 张基础卡牌加入 Fuse 搜索；觉醒和精英形态子页暂不单独加入。
- [ ] 榜单具有真实统计数据后，将卡组榜单和关联攻略加入 Fuse 搜索。
- 为高频榜单卡组关联原创打法与环境报告。

## 12. 验收状态

- [x] 卡牌列表、126 张基础卡牌详情及 55 个已上线形态页面可直接访问。
- [x] 皇室战争频道入口可到达卡牌百科；榜单入口仅在真实数据可用后正式开放。
- [x] 卡牌列表和详情页在桌面与移动端无横向溢出、文字遮挡或错位。
- [x] `hugo --minify` 构建通过，卡牌完整审计通过。
- [x] 静态验证数据可被未来 API 输出替换，不依赖模板硬编码卡名。
- [ ] 每个正式榜单条目包含 8 张有效卡牌、样本量、使用率、胜率、周期及数据更新时间。
- [ ] 正式榜单只使用真实统计快照，不以编辑样例代替 Top 榜单数据。
